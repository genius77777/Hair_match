...
def transform(neutral_model_path,
    ...
    if type(hair_mesh_paths) != list():
        hair_mesh_paths = [hair_mesh_paths]
    if type(out_mesh_paths) != list():
        out_mesh_paths = [out_mesh_paths]
    # head_model_path = fr"../../model"
    # hair_model_dir = fr"../hair_model"
    root_save_dir = fr"{tmp_dir}/../hair_data/hair_mesh_roots"
    knot_path = fr'{tmp_dir}/../hair_data/knots_mesh.txt'
    knot_id = np.loadtxt(knot_path,int).tolist()
    knot_id = np.array(knot_id)

    neutral_model = read_obj_ntb(neutral_model_path, if_nt = True)
    ...
    for id_index, hair_mesh_path in enumerate(hair_mesh_paths):
        
        identity = os.path.basename(hair_mesh_path)[:-4]
        hair_model = read_obj_ntb(hair_mesh_path, if_nt = False)
        roots_data = np.load(fr"{root_save_dir}/{identity}.npy", allow_pickle=True).item()
        ...
        RT_gpu = torch.tensor(RT, dtype = torch.float16).to(device = 'cuda')
        positions_np = hair_model.vs
        positions_np_homo = np.concatenate([positions_np, np.ones((positions_np.shape[0], 1))], axis=1)
        positions_np_homo_gpu =  torch.tensor(positions_np_homo, dtype = torch.float16).to(device = 'cuda')
        positions_np_homo_transformed_gpu = RT_gpu @ positions_np_homo_gpu[:,:,None]
        root_position_np_gpu = positions_np_homo_transformed_gpu[:, :3, 0].reshape(-1, 3)
